...
char          multi_select_string[FIELDRANGELENGTH];

/*VARARGS*/
popmenu (va_list *va_alist, ...)
{
        va_list     ap;
        int         action;
        int         MenuId;
        ...
        int        _runMenu();
        int        field_multi_select;


        va_start (ap, va_alist);
        action = va_alist;

        field_multi_select = 0;
           ...
           */
           ...
           case CREATEMENU:
                ...
                if (NewMenu == TRUE)
                        ...
                        {
                                idx = ++midx;
                                if (debug) fprintf(stderr, "\npopmenu.c:\tMenu %d out of a maximum of %d", idx, MAXMENUS);
                        }
                        else
                        {
                                printf("%c", 7); /* Beep */;
                ...
                if (debug)
                        fprintf(stderr, "\npopmenu.c:\t1 THEmenu[idx]->width = %d, THEmenu[idx]->winSize = %d, row = %d, col = %d",THEmenu[idx]->width, THEmenu[idx]->winSize, THEmenu[idx]->row, THEmenu[idx]->col);

                if (THEmenu[idx]->width > COLS)
                {
                        printf("%c", 7); /* Beep */;
                        mvprintw (22, 0, "Popmenu width (%d) exceeded (%d)", COLS, THEmenu[idx]->width);
                        if (debug) fprintf(stderr, "\npopmenu.c:\tPopmenu width (%d) exceeded (%d)", COLS, THEmenu[idx]->width);
                        shutdown ();
                }
                ...
                */
                ...
                if ( wssize > 0 )
                {
                        ...
                        field_multi_select = va_arg (ap, char *);
                        if (debug) fprintf(stderr, "\npopmenu.c:\t1 field_multi_select = %d", field_multi_select);
                        THEmenu[idx]->field_multi_select = field_multi_select;;
                        while (*ws != NULL)
                        {
                                ...
                                if (strlen(ws) > THEmenu[idx]->width)
                                {
                                        ...
                                        if (debug) fprintf(stderr, "\npopmenu.c:\t2 THEmenu[idx]->width = %d",THEmenu[idx]->width);
                                }
                                ws += wssize ;
                        }
                }
                else                /* this is basically dup code as above */
                {
                        ...
                        field_multi_select = va_arg (ap, char *);
                        if (debug) fprintf(stderr, "\npopmenu.c:\t2 field_multi_select = %d", field_multi_select);
                        THEmenu[idx]->field_multi_select = field_multi_select;;
                        while (*wws != NULL)
                        {
                                THEmenu[idx]->NumItems++;
                                ...
                                if (strlen(*wws) > THEmenu[idx]->width) 
                                {
                                        ...
                                        if (debug) fprintf(stderr, "\npopmenu.c:\t3 THEmenu[idx]->width = %d",THEmenu[idx]->width);
                                }
                                wws++ ;
                        }
                }
...
*/
                ...
                for (i = 0; i <= midx; i++)
                        if (THEmenu[i]->MenuId == MenuId)
                        {
                                drawbox( THEmenu[i]->win, 1, 1,
                                         ...
                                         THEmenu[i]->width+4,
                                         SingleLine, DumbLine,
                                        (THEmenu[i]->winSize >= 4  && THEmenu[i]->NumItems+1 >
                                                THEmenu[i]->winSize),
                                        (THEmenu[i]->helpfile !=
                                                (char *)NULL) ) ;

                                mvwprintw (THEmenu[i]->win, 0, 
                                          ((THEmenu[i]->width+4 - strlen(THEmenu[i]->title))/2),
                                          "%s",
                                          THEmenu[i]->title);
           ...
           default:
                ...
                */
                ...
                if (debug > 8) fprintf(stderr, "\npopmenu.c\tcalling _runMenu(THEmenu[%d]=%d, field_multi_select=%d)", i, THEmenu[i], field_multi_select);
        ...
        MENU            *menu;
{
        ...
        int             multi_index;
        char            toremove[FIELDRANGELENGTH];

        if (debug > 8) fprintf(stderr, "\npopmenu.c:\t_runMenu Here we are");
        multi_select_string[0]='\0';            /* nullify string */
        multi_index=0;
        ...
        */
        ...
        while (1)
        {
                ...
                flushinp();
                if (debug) fprintf(stderr, "\npopmenu.c:\t_runMenu 1 waiting for input within _runMenu");
                ch = wgetch (menu->win);
                if (ch == ERR) 
                {
                        if (debug) fprintf(stderr, "\npopmenu.c:\t_runMenu 2 waiting for input within _runMenu");
                        exit(1);
                }
                ...
                switch ( ch )
                {
                   ...
                   default:
                        ...
                        /* add or remove an asterisk against the item if field_multi_select is set  */
                        /* also copy/remove the string to/from the field_multi_select_string buffer */
                        if (ch == ' ' && menu->field_multi_select == 1)
                        {
                                i=0;
                                wmove(menu->win, fptr-top+1, 1);
                                if (winch(menu->win) != '*')
                                {
                                        ch="*";
                                        mvwprintw (menu->win, fptr-top+1, 1, "%s", ch);
                                        while (ch != ' ')
                                        {
                                                ch = winch (menu->win) & A_CHARTEXT;
                                                multi_select_string[multi_index++] = ch;
                                                multi_select_string[multi_index] = '\0';
                                                wmove(menu->win, fptr-top+1, 3+i++);
                                        }
                                        if (debug) fprintf(stderr, "\npopmenu.c:\tItem selected: multi_select_string=%s", multi_select_string);
                                }
                                else
                                {
                                        ch=" ";
                                        ...
                                        if (debug) fprintf(stderr, "\npopmenu.c:\tcalling removeSubstring(%s, %s)", multi_select_string, toremove);
                                        removeSubstring(multi_select_string, toremove);
                                        if (debug) fprintf(stderr, "\npopmenu.c:\tItem de-selected: multi_select_string=%s", multi_select_string);
                                        refresh();
                                }
        ...
        int        top ;
{
        ...
        for (i=0; i < menu->winSize && top+i-1 < menu->NumItems+1; i++)
	{
                if (debug) fprintf(stderr, "\npopmenu.c:\t_showWin(%d, %d) mvwprintw (%d, %d, %d, %s, %d, %s)", menu, top,  menu->win, i+1, 2, "%-*s", menu->width, menu->items[top+i-1]);
...
removeSubstring(char *s,const char *toremove)
{
        if (debug) fprintf(stderr, "\npopmenu.c:\tremoveSubstring(%s, %s)", s, toremove);
